...
int main(int argc, char* argv[]) {
	...
	glm::mat4 mp = Transform::lookAtMatrix(glm::vec3(3, 4, 1), glm::vec3(0, 0, 0), glm::vec3(0, 1, 0));
	for (int i = 0; i < obj.mesh.vertices.size(); i++) {
		glm::vec4 sol = glm::vec4(obj.mesh.vertices[i], 1.0);
		sol = mp * sol;
		std::cout << glm::to_string(obj.mesh.vertices[i]) << glm::to_string(sol) << std::endl;
	}*/
	return EXIT_SUCCESS;
}