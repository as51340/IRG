...
int main(int argc, char* argv[]) {
	...
	Mesh mesh;
	initMesh(mesh, argv);
	//std::cout << mesh.resPath << " " << mesh.vertices.size() << " " << mesh.indices.size() << std::endl;
	...
	Renderer* renderer = new Renderer(mWidth, mHeight, objects, argv[0]); //renderer ce mi ga initat
	delete renderer;
	return EXIT_SUCCESS;
}