...
void Object::transformObject() {
	...
	transform.updateModel(trans_mat);
	...
	transform.updateModel(scaling); //pa zato jer je rijeè o inverzu inace je okej
	//this->mesh.applyTransform(scaling * trans_mat);
}
...
void Object::loadShader(char* path, char* naziv) {
	...
	std::string pathVert;
	std::string pathFrag;
	std::string pathGeom; //new

	pathVert.append(path, sPath.find_last_of("\\/") + 1);
	pathFrag.append(path, sPath.find_last_of("\\/") + 1);
	pathGeom.append(path, sPath.find_last_of("\\/") + 1); //new
	if (pathFrag[pathFrag.size() - 1] == '/') {
		pathVert.append("shaders/");
		pathFrag.append("shaders/");
		pathGeom.append("shaders/"); //new
	}
	...
	pathGeom.append(naziv); //new
	pathGeom.append(".geom"); //new

	sjencar = new Shader(pathVert.c_str(), pathFrag.c_str(), pathGeom.c_str());
}
